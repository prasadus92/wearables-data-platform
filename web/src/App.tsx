...
function AppShell() {
  ...
  const [sessions, setSessions] = useState<Record<AggregatorEnv, StoredUser | null>>(() => ({
  ...
  useEffect(() => {
    ...
    if (clerkLoaded) {
      const next: AggregatorEnv = signedIn ? 'production' : 'sandbox'
      if (mode !== next) {
        setMode(next)
    ...
    return (
      <MotionConfig reducedMotion="user">
        ...
        >
          ...
          >
            <img src="/examplehealth-logo.svg" alt="ExampleHealth" className="h-6 w-auto" />
        ...
        <header className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <h1>
              ...
              >
                <img src="/examplehealth-logo.svg" alt="ExampleHealth" className="h-4 w-auto" />
