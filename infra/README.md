...
```sh
AWS_PROFILE=default ./infra/deploy.sh          # image build + push + service roll
APPLY=1 AWS_PROFILE=default ./infra/deploy.sh  # terraform apply first
```
...
`prasadus92/wearables-data-platform`. No AWS keys are stored in GitHub.
...
1. Apply the infrastructure (creates bucket, distribution, OIDC provider,
   ...
   ```sh
   APPLY=1 AWS_PROFILE=default ./infra/deploy.sh
   ```
