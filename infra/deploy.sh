...
# Aggregator credentials come from the local .env (never committed).
AGGREGATOR_API_KEY=$(grep '^AGGREGATOR_API_KEY=' .env | cut -d= -f2)
...
aws ecs update-service --cluster wearables-data-platform --service api --force-new-deployment \
  --region "$REGION" --profile "$PROFILE" --no-cli-pager --query 'service.serviceName'
aws ecs update-service --cluster wearables-data-platform --service worker --force-new-deployment \
  --region "$REGION" --profile "$PROFILE" --no-cli-pager --query 'service.serviceName'
