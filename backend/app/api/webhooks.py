"""Inbound Junction webhooks.

Design constraints (from Junction's delivery semantics):
...
def verify_signature(raw_body: bytes, headers: Mapping[str, str]) -> None:
    ...
    Junction signs per webhook endpoint, and each environment (sandbox,
...
@router.post("/junction", status_code=status.HTTP_202_ACCEPTED)
async def junction_webhook(request: Request, db: DbSession) -> dict:
    """Receive one Junction event: verify the Svix signature, persist the
    raw payload (deduped on the Svix message id), enqueue processing, and
    ACK with 202 well inside Junction's 15-second delivery timeout."""
