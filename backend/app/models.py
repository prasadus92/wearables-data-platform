...
- ``users``: app users mapped 1:1 to a Aggregator user.
- ``connections``: a user's link to one wearable provider (whoop, oura…).
- ``webhook_events``: raw inbound Aggregator events. Source of truth for
...
class User(Base):
    ...
    # Stable external identifier we hand to Aggregator as client_user_id.
...
class Connection(Base):
    """A user's link to a wearable provider via Aggregator."""
    ...
    provider: Mapped[str] = mapped_column(String(64))  # aggregator provider slug, e.g. "whoop_v2"
...
class WebhookEvent(Base):
    """Raw Aggregator webhook payloads, persisted before processing.
...
class DeviceEvent(Base):
    ...
    aggregator_user_id: Mapped[str | None] = mapped_column(String(255))
