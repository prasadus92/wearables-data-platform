"""Normalization of Aggregator webhook events into our domain model.
...
# Aggregator resource slug -> our metric. blood_oxygen is Aggregator's name for SpO2.
...
@dataclass
class IngestPlan:
    """Everything a single webhook event implies. ``aggregator_user_id`` /
...
def parse_event(payload: dict) -> IngestPlan:
    ...
    aggregator_user_id = payload.get("user_id") or data.get("user_id")
