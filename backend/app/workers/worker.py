    ...
    if plan.aggregator_user_id:
        user = (
            await session.execute(
                select(User).where(User.aggregator_user_id == plan.aggregator_user_id)
            )
        ).scalar_one_or_none()
        if user:
            return user
    ...
    async with db_session() as session:
        ...
        try:
            ...
            if user is None:
                ...
                # Aggregator dashboard). Park it as failed for replay once the
...
async def process_backfill(
    ...
    """Pull historical data from Aggregator's REST API and upsert it.
    ...
    async with db_session() as session:
        ...
        if resource == SLEEP_RESOURCE:
            while True:
                page = await aggregator.get_sleep_summary(
                    user.aggregator_user_id,
                    start_date,
                    end_date,
                    provider=provider,
                    next_cursor=cursor,
                ...
                plan = parse_event(
                    {
                        ...
                        "user_id": user.aggregator_user_id,
                        "data": {"data": sessions},
                    }
                )
