...
async def test_guest_token_issued_once_and_grants_own_access(
    with_auth, stub_junction, client, engine
):
...
async def test_guest_token_cannot_read_another_user(with_auth, stub_junction, client):
...
async def test_garbage_token_rejected(with_auth, stub_junction, client):
...
async def test_guest_token_rejected_on_me(with_auth, stub_junction, client):
    guest = await _mint_guest(client)
    response = await client.post("/v1/me", headers=bearer(guest["guest_token"]))
    assert response.status_code == 403


async def test_guest_token_cannot_erase_own_user(with_auth, stub_junction, client):
...
async def test_service_key_unaffected(with_auth, stub_junction, client):
    guest = await _mint_guest(client)
...
async def test_guest_creation_still_writes_ledger(with_auth, stub_junction, client, engine):
    guest = await _mint_guest(client)
