...
deregistered at Junction (non-disconnected only), the Junction user is
...
deliberately survive as the ingestion audit log. Junction 404s are
...
class StubJunction:
    ...
    async def deregister_provider(self, junction_user_id: str, provider: str) -> dict:
        ...
        if self.gone_upstream:
            raise JunctionError(404, "user not found")
        return {"success": True}

    async def delete_user(self, junction_user_id: str) -> dict:
        self.deleted.append(junction_user_id)
        if self.gone_upstream:
            raise JunctionError(404, "user not found")
        return {"success": True}
        ...
        session.add_all(
            [
                ...
                WebhookEvent(
                    ...
                    payload={"user_id": body["junction_user_id"]},
                ),
            ]
        )
...
async def test_service_key_erases_user_and_cascades(with_auth, stub_junction, client, engine):
    ...
    # Junction cleanup: only the non-disconnected provider, then the user.
    assert stub_junction.deregistered == [(body["junction_user_id"], "oura")]
    assert stub_junction.deleted == [body["junction_user_id"]]
...
async def test_erasure_tolerates_junction_404s(with_auth, monkeypatch, client, engine):
    stub = StubJunction(gone_upstream=True)
    monkeypatch.setattr(users_module, "junction_client_for", lambda env: stub)

    body = await _seed_user_with_data(client, engine)
    response = await client.delete(f"/v1/users/{body['id']}", headers=SERVICE)
    assert response.status_code == 204
    assert stub.deleted == [body["junction_user_id"]]
    assert (await client.get(f"/v1/users/{body['id']}", headers=SERVICE)).status_code == 404


async def test_erasing_unknown_user_is_404(with_auth, stub_junction, client):
    response = await client.delete(f"/v1/users/{uuid.uuid4()}", headers=SERVICE)
    assert response.status_code == 404
    assert stub_junction.deregistered == []
    assert stub_junction.deleted == []


async def test_user_without_junction_mapping_still_erased(with_auth, stub_junction, client, engine):
    factory = async_sessionmaker(engine, expire_on_commit=False)
    async with factory() as session:
        user = User(client_user_id="never-registered", junction_user_id=None)
        session.add(user)
        await session.commit()
        user_id = str(user.id)
