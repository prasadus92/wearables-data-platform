...
class StubAggregator:
    # aggregator_user_id is unique per user, so derive it from the input.
...
async def test_valid_jwt_bootstraps_me(with_clerk, stub_aggregator, client, rsa_key):
    response = await client.post("/v1/me", headers=bearer(make_jwt(rsa_key)))
    assert response.status_code == 200
    body = response.json()
    assert body["client_user_id"] == f"clerk:{SUB}"
    assert body["aggregator_user_id"] == f"jnc-test-clerk:{SUB}"
    assert body["aggregator_environment"] == "sandbox"
...
async def test_me_production_gets_suffixed_identity(with_clerk, stub_aggregator, client, rsa_key):
    ...
    assert body["aggregator_environment"] == "production"
...
async def test_user_token_scoped_to_own_user(with_clerk, stub_aggregator, client, rsa_key):
...
async def test_service_key_keeps_full_access(with_clerk, stub_aggregator, client, rsa_key):
    own = await client.post("/v1/me", headers=bearer(make_jwt(rsa_key)))
    own_id = own.json()["id"]
