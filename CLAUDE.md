...
Working notes for AI agents in this repo. Humans: start with README.md.
...
- Deploy: `AWS_PROFILE=default ./infra/deploy.sh` (add `APPLY=1` when Terraform changed)
- Migrations: alembic, autogenerate from models, applied on container start
- Aggregator link URLs: `./scripts/make-link.sh <provider>` (expire in 60 minutes)
...
- User-facing copy never names vendors ("Aggregator") or environments ("sandbox");
...
- Aggregator: webhook receiver must ACK in <15s (8 retries, then endpoint disabled);
...
- Dual environment: users carry aggregator_environment; clients/webhook secrets resolve
...
- Real wearables deliver heart rate, HRV, and breathing rate inside Aggregator
...
- Demo mode seeds synthetic breathing rate and blood pressure at demo connect
  (app/services/demo_seed.py); Aggregator demo data covers only HR/HRV/SpO2.
