...
- **Consent ledger and erasure.** `device_events` records every connect, disconnect, and identity change with actor attribution; one service call erases a user locally and at Aggregator.
- **Both Aggregator environments through one deployment.** Users carry their environment; webhooks, clients, and secrets resolve per environment.
