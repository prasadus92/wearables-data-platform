#!/usr/bin/env bash
# Generate a Junction Link URL for connecting a real wearable account.
#
# Usage:
...
# Production EU key, from .env (JUNCTION_PROD_API_KEY) or environment.
cd "$(dirname "$0")/.."
KEY=${JUNCTION_PROD_API_KEY:-$(grep '^JUNCTION_PROD_API_KEY=' .env | cut -d= -f2)}
BASE=https://api.eu.junction.com

# Resolve or create the Junction user for this client_user_id.
