#!/usr/bin/env bash
# Seed the shared sample account: a sandbox user named "youth-sample" with a
# demo wearable connected and a synced 30-day history. Safe to rerun.
#
# Usage:
#   ./scripts/seed-sample.sh                          # local backend
#   API=https://api.youth.luminik.io ./scripts/seed-sample.sh   # deployed
set -euo pipefail
...
USER_JSON=$(curl -s "${H[@]}" -X POST "$API/v1/users" -d '{"client_user_id": "youth-sample"}')
