# White-labeling the YOU(th) app for clinic partners
...
One public "YOU(th) Health Hub" app. The clinic partner code (already in the product:
...
flowchart LR
    ...
    B -- none --> E[Default YOUth branding]
```
...
- iOS **alternate app icons** soften the icon limitation: ship clinic icons in the binary
  ...
  separate app).
- Concession: the App Store listing and home-screen *name* stay YOU(th).
...
Apple's sanctioned channel for exactly this B2B case. YOU(th) publishes a
clinic-branded variant (own name, icon, bundle ID) as a **private custom app under
YOU(th)'s own developer account**, visible only to that clinic's organization via Apple
Business Manager. Guideline 4.2.6 does not apply because the app never enters the public store.
...
- Review: custom apps go through standard App Review under YOU(th)'s account (days,
...
- Contractually scope it (e.g. enterprise tier only), since each Tier 3 app adds a real
...
|---|---|---|---|---|
| **Tier 1** multi-tenant | In-app only (name/icon stay YOUth) | None | Hours | None |
