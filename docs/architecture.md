...
flowchart TB
    ...
    subgraph Aggregator["Aggregator (EU)"]
    ...
    DEVICES[WHOOP / Oura / Garmin / Apple Watch] --> Aggregator
...
**Webhook receiver does almost nothing.** Aggregator retries 8 times with a 15 second
...
EU region matches Aggregator EU data residency, which matters for health data under GDPR.
...
- **Multi-region**: EU and US cells matching Aggregator's regions and data-residency
...
- **GDPR**: EU region pinning, user deletion cascades (FK `ON DELETE CASCADE` plus
  Aggregator-side deregistration), raw event archives with TTL.
