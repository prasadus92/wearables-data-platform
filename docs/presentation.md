...
- Users connect Oura / WHOOP / Garmin / Fitbit via Aggregator's hosted OAuth Link;
  Apple Watch pairs through the Aggregator Connect app with a single-use code (HealthKit).
...
- Timeline API with server-side bucketing feeds the web dashboard at
  `app.examplehealth.example.com` and an Expo app sharing one TypeScript core package;
...
- Consent ledger and GDPR erasure: device_events records every connect, disconnect,
  ...
  and at Aggregator.
- Both Aggregator environments at once: demo data and real production devices (a real
...
Aggregator mobile SDK instead of the Aggregator Connect bridge app, EAS release builds in
