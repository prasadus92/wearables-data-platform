...
```bash
curl -s https://api.youth.luminik.io/health            # expect {"status":"ok",...}
...
through Junction's hosted Link page:
...
2. Open it in a browser. Junction forwards to the provider's OAuth consent screen.
...
4. Success screen appears; the connection now exists in the Junction team.
5. Verify data server-side:

```bash
KEY=$(grep '^JUNCTION_PROD_API_KEY=' .env | cut -d= -f2)
curl -s "https://api.eu.junction.com/v2/user/resolve/prasad-prod-real-devices" \
...
```bash
API=https://api.youth.luminik.io
# 1. Create a user (registers with Junction automatically)
...
# 3. Wait ~2 minutes. Junction sends provider.connection.created, then
...
Junction Dashboard, team "Prasad's Onsite", pick environment (Sandbox/Production):
Webhooks → Add Endpoint → `https://api.youth.luminik.io/webhooks/junction` →
subscribe to all event types → copy the **Signing Secret** (`whsec_...`).

Then store it and roll the services:

```bash
# put the secret in .env as JUNCTION_WEBHOOK_SECRET, then:
AWS_PROFILE=luminik APPLY=1 ./infra/deploy.sh
```
...
AWS_PROFILE=luminik aws logs tail /ecs/youth-wearables-api --region eu-central-1 --follow

# Watch the worker processing
AWS_PROFILE=luminik aws logs tail /ecs/youth-wearables-worker --region eu-central-1 --follow
...
Open app.youth.luminik.io in a private window: the landing shows Sign in and Try
...
1. Sign in to the app, open Devices, pick a provider. Junction Link opens; sign in
...
3. **Data unlock, the one step software cannot do:** open the provider's own phone
   ...
   cloud. Junction only sees what the vendor cloud has. A web sign-in to the vendor
...
1. Install **Vital Connect** from the App Store on the iPhone paired with the watch.
...
```bash
cd infra/terraform
AWS_PROFILE=luminik terraform destroy   # removes ALB, ECS, RDS, Redis, everything
```

Also rotate/delete afterwards: the Junction API keys, the WHOOP/Oura account password
...
- Junction sandbox caps at 50 users; demo users expire after 7 days.
