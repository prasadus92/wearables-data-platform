...
- Own team "Prasad's Onsite" (org: ExampleHealth Aggregator org, Prasad as Org Admin), under
...
- URL (same for both): `https://api.examplehealth.example.com/webhooks/aggregator`
...
- Copied each environment's Svix signing secret into `.env`
  (`AGGREGATOR_WEBHOOK_SECRET` = sandbox, `AGGREGATOR_PROD_WEBHOOK_SECRET` = production),
...
WHOOP refuses aggregator-shared credentials, so every Aggregator team needs its own WHOOP
...
2. Created app "Prasad's Onsite (Test App)" with:
   - Redirect URIs (must exactly match what Aggregator sends):
     `https://api.eu.aggregator.com/v2/link/connect/whoop_v2`
   ...
   - Webhook URL: left empty (WHOOP-to-Aggregator delivery is Aggregator's concern).
3. Aggregator Dashboard → Production → Config → Custom Credentials → Whoop V2 → Setup:
...
- Connections happen via Aggregator's hosted Link page, never the provider dashboards:
...
- Account: existing org SSO (`AWS_PROFILE=default`), region eu-central-1 to match the
  Aggregator EU data residency.
- Everything via Terraform (`infra/terraform`): ECS Fargate (api x2, worker), RDS
  ...
  `api.examplehealth.example.com` on an existing hosted zone, ECR, SSM SecureString secrets,
  CloudWatch logs.
...
- `terraform destroy` (infra), delete the WHOOP dev app (or rotate its secret), rotate
  or delete both Aggregator API keys, change the shared device-account password, remove
